...
export function buildGroupMatches(){
  ...
  Object.entries(GROUPS).forEach(([g,teams],gi)=>{
    RR.forEach((day,di)=>{
      day.forEach(([hi,ai])=>{
        const date=new Date(2026,5,11+gi+di*4,18,0);
...
export function seedResults(matches){
  ...
  matches.forEach((m)=>{
    ...
    if(m.matchday===1&&played.includes(m.group)){
      ...
      const live=(m.group==="C"&&m.id.includes("BRA"))||(m.group==="D"&&m.id.includes("USA"));
      if(live){m.status="live";m.minute=m.group==="C"?57:34;m.homeScore=1;m.awayScore=m.group==="C"?0:1;}
...
export function buildKnockout(){
  ...
  const mk=(phase,n,lh,la,off,st)=>ko.push({id:`${phase}-${n}`,phase,n,matchday:null,
    home:null,away:null,placeholderHome:lh,placeholderAway:la,
    datetime:new Date(2026,5,28+off,18,0).toISOString(),
